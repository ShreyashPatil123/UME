...
void EpochRcu::enter(size_t slot_index) noexcept {
    if (slot_index >= kMaxRcuThreads) return;

    auto& slot = slots_[slot_index];
    const uint64_t current_g_epoch = global_epoch_.load(std::memory_order_relaxed);

    slot.entry_time_ns.store(platform::to_ns(platform::monotonic_now_ns()), std::memory_order_relaxed);
...
void EpochRcu::retire(std::function<void()> deleter) {
    ...
    retired_list_.push_back(DeferredDeletion{
        .epoch = current_g_epoch,
...
bool EpochRcu::try_advance_epoch() noexcept {
    const uint64_t current_g_epoch = global_epoch_.load(std::memory_order_relaxed);
    const size_t num_slots = std::min(allocated_slots_.load(std::memory_order_relaxed), kMaxRcuThreads);
    ...
    for (size_t i = 0; i < num_slots; ++i) {
        ...
        if (slot.active.load(std::memory_order_acquire)) {
            ...
            if (thread_epoch < current_g_epoch) {
                ...
                const uint64_t elapsed_ms = (now_ns > entry_ns) ? (now_ns - entry_ns) / 1'000'000ULL : 0;
