...
IntType get_int(const toml::value& v, const std::string& key, IntType default_val) {
    if (v.contains(key)) {
        return static_cast<IntType>(toml::find_or<std::int64_t>(v, key, static_cast<std::int64_t>(default_val)));
    }
    return default_val;
}
...
Result<Config> Config::load_from_file(const std::string& path) {
    ...
    try {
        ...
        if (data.contains("engine")) {
            ...
            config.engine.overhead_budget_cpu_percent = get_float(sec, "overhead_budget_cpu_percent", config.engine.overhead_budget_cpu_percent);
            config.engine.overhead_budget_memory_mb = get_int(sec, "overhead_budget_memory_mb", config.engine.overhead_budget_memory_mb);
        }
        ...
        if (data.contains("journal")) {
            ...
            config.journal.segment_size_bytes = get_int(sec, "segment_size_mb", default_mb) * 1024 * 1024;
            config.journal.hot_retention_seconds = get_int(sec, "hot_retention_seconds", config.journal.hot_retention_seconds);
            config.journal.warm_retention_seconds = get_int(sec, "warm_retention_seconds", config.journal.warm_retention_seconds);
            config.journal.compaction_enabled = get_bool(sec, "compaction_enabled", config.journal.compaction_enabled);
        }

        // [object_store]
        if (data.contains("object_store")) {
            const auto& sec = toml::find(data, "object_store");
            config.object_store.shard_count = get_int(sec, "shard_count", config.object_store.shard_count);
            config.object_store.small_alloc_threshold_bytes = get_int(sec, "small_alloc_threshold_bytes", config.object_store.small_alloc_threshold_bytes);
            config.object_store.tombstone_reap_interval_seconds = get_int(sec, "tombstone_reap_interval_seconds", config.object_store.tombstone_reap_interval_seconds);
        }

        // [graph]
        if (data.contains("graph")) {
            const auto& sec = toml::find(data, "graph");
            config.graph.rcu_epoch_timeout_ms = get_int(sec, "rcu_epoch_timeout_ms", config.graph.rcu_epoch_timeout_ms);
        }

        // [probes]
        if (data.contains("probes")) {
            const auto& sec = toml::find(data, "probes");
            config.probes.sample_rate_hz = get_int(sec, "sample_rate_hz", config.probes.sample_rate_hz);
        ...
        if (data.contains("adapters")) {
            ...
            config.adapters.python_gil_max_hold_us = get_int(sec, "python_gil_max_hold_us", config.adapters.python_gil_max_hold_us);
            config.adapters.python_batch_hz = get_int(sec, "python_batch_hz", config.adapters.python_batch_hz);
        }

        // [dashboard]
        if (data.contains("dashboard")) {
            const auto& sec = toml::find(data, "dashboard");
            config.dashboard.websocket_port = get_int(sec, "websocket_port", config.dashboard.websocket_port);
            config.dashboard.rest_port = get_int(sec, "rest_port", config.dashboard.rest_port);
            config.dashboard.max_clients = get_int(sec, "max_clients", config.dashboard.max_clients);
        }

        // [prediction]
        if (data.contains("prediction")) {
            const auto& sec = toml::find(data, "prediction");
            config.prediction.cycle_interval_ms = get_int(sec, "cycle_interval_ms", config.prediction.cycle_interval_ms);
            config.prediction.models_enabled = get_str(sec, "models_enabled", config.prediction.models_enabled);
        }

        // [advisor]
        if (data.contains("advisor")) {
            const auto& sec = toml::find(data, "advisor");
            config.advisor.cycle_interval_ms = get_int(sec, "cycle_interval_ms", config.advisor.cycle_interval_ms);
...
Status Config::validate() const noexcept {
    // Engine validation
    if (engine.overhead_budget_cpu_percent < 0.0f ||
        engine.overhead_budget_cpu_percent > 100.0f) {
        return Status::kInvalidConfig;
    }
    ...
    if (object_store.shard_count == 0 ||
        !std::has_single_bit(object_store.shard_count)) {
        return Status::kInvalidConfig;
    }
